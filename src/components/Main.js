    ...
    <section className="profile">
      ...
      <div className="profile__info">
        <div className="profile__edition">
          ...
          <button type="button" name="editButton" className="profile__button profile__button_edit" onClick={props.onEditProfile}></button>
        </div>
        <p className="profile__subtitle">{userDescription}</p>
      </div>
      <button type="button" name="addButton" className="profile__button profile__button_add" onClick={props.onAddPlace}><img src={addButtonPath} alt="Кнопка добавления" className="profile__icon" /></button>
    </section>

    <section className="elements">
      {cards.map((el, i) => (
        <div key={el._id}>
          <Card card={el} onCardClick={props.onCardClick} />
        </div>
      ))}
    </section>
  </main>
